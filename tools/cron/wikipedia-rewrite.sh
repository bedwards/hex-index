...
log "=== Job 3 complete ($(( RUN_E/60 ))m $(( RUN_E%60 ))s) ==="
ln -sf "$LOG_FILE" "$PROJECT_DIR/logs/wiki-rewrite-latest.log"
find "$PROJECT_DIR/logs" -name "wiki-rewrite-*.log" -not -name "wiki-rewrite-latest.log" -mtime +7 -delete
