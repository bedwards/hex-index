...
# Walks articles with only excerpts, rewrites full text via LLM
...
# Run rewrites
step_start "Article rewrite"
timeout 10800 npx tsx tools/jobs/article-rewrite.ts --limit 100 2>&1 | tee -a "$LOG_FILE" || {
    EC=$?
...
if git diff --quiet docs/ && git diff --quiet --cached docs/; then
    ...
    git add docs/
    git commit -m "feat: article rewrites $(date +%Y-%m-%d\ %H:%M)" 2>&1 | tee -a "$LOG_FILE"
    for i in 1 2 3; do
