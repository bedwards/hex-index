...
async function renderArticle(id: string): Promise<string> {
  try {
    ...
    // SECURITY NOTE: Content in /library/ is trusted - it's HTML generated by our own
    // ingestion pipeline from markdown files. The pipeline uses marked.js which outputs
    // safe HTML from markdown. No user-submitted content is served from this endpoint.
    ...
    if (article.content_path) {
      try {
        ...
        // Content not available - use default placeholder
      }
    }
